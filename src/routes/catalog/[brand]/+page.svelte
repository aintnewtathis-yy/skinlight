<script>
	...
	function gotoNextPage() {
		if (currentCategory) {
			searchParams.set('categorySlug', currentCategory);
		}
		if (currentLine) {
			searchParams.set('lineSlug', currentLine);
		}
		if (maxPriceFilter) {
		...
		goto(`/catalog/${$page.params.brand}?${searchParams.toString()}`, {
			replaceState: true,
			noScroll: true
		});
	}
...
{#snippet sidebarSnippet(content)}
	...
	<div
		class="sidebar sticky bottom-5 top-[76px] -ml-2 flex h-fit max-h-[calc(100dvh-76px)] flex-col gap-5 overflow-y-auto"
	>
	...
	<div class="container">
		<div class="flex flex-col gap-10">
			<div class="max-lg:hidden">
				<Breadcrumbs brand={{ name: data.currentBrand?.name, href: data.currentBrand?.seo.slug }} />
			</div>
